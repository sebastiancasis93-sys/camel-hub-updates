...
if rootWidget then
  ...
  for _, widgetId in ipairs({"lblPos", "lblPosX", "ipx", "lblPosY", "ipy", "createIcon"}) do
    if spellListWindow[widgetId] then
      spellListWindow[widgetId]:hide()
    end
  end
  ...
  AutoBuff = {
    ...
    setOn = function()
      config.enabled = true
      buffUi.status:setOn(true)
    end,
    setOff = function()
      config.enabled = false
      buffUi.status:setOn(false)
    end
  }
