...
local CAMEL_HUB_ACCENT = "#2de0d7"
UI.Separator()
local camelPartyLabel = UI.Label("CAMEL HUB PARTY")
if camelPartyLabel and camelPartyLabel.setColor then camelPartyLabel:setColor(CAMEL_HUB_ACCENT) end
...
if rootWidget then
  ...
  if autopartyui.status.setColor then autopartyui.status:setColor(CAMEL_HUB_ACCENT) end
  if autopartyui.editPlayerList.setColor then autopartyui.editPlayerList:setColor(CAMEL_HUB_ACCENT) end
  if autopartyui.ptShare.setColor then autopartyui.ptShare:setColor(CAMEL_HUB_ACCENT) end
...
-- Controller API used by Camel Hub floating icons.
AutoParty = AutoParty or {}
function AutoParty.isOn()
  return config.enabled == true
end
function AutoParty.setOn()
  config.enabled = true
  if autopartyui and autopartyui.status then autopartyui.status:setOn(true) end
end
function AutoParty.setOff()
  config.enabled = false
  if autopartyui and autopartyui.status then autopartyui.status:setOn(false) end
end
function AutoParty.show()
  if autoPartyListWindow then
    autoPartyListWindow:show()
    autoPartyListWindow:raise()
    autoPartyListWindow:focus()
  end
